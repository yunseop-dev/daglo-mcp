  ...
  private registerTools() {
    this.server.registerTool(
      'get-boards',
      {
        ...
        inputSchema: {
          ...
          checkedFilter: z.enum(['incompleteRecording', 'isPdf']).optional().describe('Filter by incomplete recordings or PDFs'),
      ...
      async (args) => {
        ...
        if (args.checkedFilter) params.append(`checkedFilter=${args.checkedFilter}`, 'true');
    ...
    this.server.registerTool(
      'get-board-detail',
      {
        ...
        description: 'Retrieve detailed information including content, summary, keywords, AI summary, and segments for a specific board. Supports filtering which data to include.',
        inputSchema: {
          boardId: z.string().describe('Board ID to fetch details for'),
          fileMetaId: z.string().optional().describe('File metadata ID (optional)'),
          includeContent: z.boolean().optional().describe('Include full transcription content (default: true)'),
          includeSummary: z.boolean().optional().describe('Include summary (default: true)'),
          includeKeywords: z.boolean().optional().describe('Include keywords (default: true)'),
          includeAiSummary: z.boolean().optional().describe('Include AI summary (default: true)'),
          includeSegments: z.boolean().optional().describe('Include timestamped segments (default: true)')
        }
      },
      async (args) => {
        const params = new URLSearchParams();
    ...
    this.server.registerTool(
      'get-folders',
      {
        ...
        inputSchema: {
          includeRoot: z.boolean().optional().describe('Include root folder (default: true)')
        }
      },
      async (args) => {
        const params = new URLSearchParams();
    ...
    this.server.registerTool(
      'login',
      {
        ...
        inputSchema: {
          email: z.string().email().optional().describe('Daglo account email (default: env DAGLO_EMAIL)'),
          password: z.string().min(1).optional().describe('Daglo account password (default: env DAGLO_PASSWORD)')
        }
      },
      async (args) => {
        ...
        if (!this.accessToken) {
          throw new Error('Login failed: access token not found in response headers.');
        }
